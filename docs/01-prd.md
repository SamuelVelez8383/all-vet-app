...
- **Data Privacy:** The data must be private and not exposed to anyone else but Sofía (or the specific user when expanded to others) and whichever developer has access to the database. Must comply with Colombian data privacy law: **Ley 1581 de 2012** (Habeas Data) and **Decreto 1377 de 2013**. In practice: users must consent to data collection, data cannot be shared with third parties without consent, and users have the right to access, correct, and delete their data. Revisit compliance requirements before expanding to additional users.
- **Loading time:** First meaningful paint under 2 seconds on a good connection; under 5 seconds on slow 3G. Only fetch what is needed at each time — lazy loading strategy throughout.
...
| **Google OAuth "unverified app" warning** | UX friction / scary consent screen on first Google sign-in for Drive access | High — guaranteed unless app is verified by Google | For MVP (single user), Sofía can click through the warning — it's a one-time friction. If the app expands to other users, submit for Google verification (requires published privacy policy + Google review). Flag as a pre-launch task before any broader rollout. |
